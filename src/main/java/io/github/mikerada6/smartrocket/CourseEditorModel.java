...
public final class CourseEditorModel {
    ...
    public Hit hitTest(double x, double y) {
        ...
        if (x >= launch.x() - EDGE_GRAB && x <= launch.x() + Rocket.WIDTH + EDGE_GRAB
                && y >= launch.y() - EDGE_GRAB && y <= launch.y() + Rocket.HEIGHT + EDGE_GRAB) {
    ...
    public void placeLaunch(double x, double y) {
        launch = new Vec2(clamp(x, 0, width - Rocket.WIDTH), clamp(y, 0, height - Rocket.HEIGHT));
        dirty = true;
    }
